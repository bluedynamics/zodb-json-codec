...
Every conversion passes through this AST.
Pickle bytes decode *into*
...
enum at 48 bytes instead of 56.
Since most `PickleValue` nodes are scalars or
...
up the `PickleValue` tree.
It maintains a stack, a memo (for shared
...
- **Value semantics on the stack.** Each `PickleValue` is owned, not
  reference-counted.
  Early experiments with `Rc<PickleValue>` showed that the
  heap allocation cost of `Rc::new` per stack push exceeded any savings from
  shared memo references.
  Most values are created once and consumed once.
- **GIL release.** The parser calls `py.detach()` before entering the
  pure-Rust parsing loop.
  No Python API calls happen during parsing, so the
...
objects via PyO3.
This is the original and most general output path.
It
...
The primary cost is string allocation: every Python
...
This path was the original JSON output method.
...
AST to a `String` buffer.
It eliminates all `serde_json::Value` intermediate
...
backend (`decode_zodb_record_for_pg_json`).
It uses a thread-local buffer that
...
without constructing a `PickleValue` tree.
It dispatches on Python type
...
tree via serde, then encodes that tree to pickle bytes via `encode.rs`.
This
is used when the input is already a JSON string (for example, from PostgreSQL).

## Known types (known_types.rs)

Certain Python types have no direct JSON equivalent: `datetime`, `date`,
`time`, `timedelta`, `Decimal`, `UUID`, `set`, `frozenset`.
These types appear
...
boundary.
On decode, it recognizes the `(module, class, args)` pattern and
emits a compact JSON marker (for example, `{"@dt:" "2025-06-15T12:00:00"}`).
On
...
internal state.
The `btrees.rs` module classifies BTree records and transforms
